...
#[derive(Error, Debug)]
pub enum OpenTxReaderError {
    #[error("Transaction read error")]
    OutOfBound,
    #[error("ItemMissing")]
    ItemMissing,
    #[error("LengthNotEnough")]
    LengthNotEnough,
    #[error("InvalidData")]
    InvalidData,
...
impl OpenTxReader {
    ...
    pub fn group_input_len(&self) -> Result<u64, OpenTxReaderError> {
        let len = self.group_input_index.len();
        let len = u64::try_from(len).map_err(|_e| OpenTxReaderError::LenOverflow(len))?;
        Ok(len)
    }

    /// get the group output length
    pub fn group_output_len(&self) -> Result<u64, OpenTxReaderError> {
    ...
    pub fn input(&self, index: usize) -> Result<CellInput, OpenTxReaderError> {
        self.transaction
            .inputs()
            .get(index)
            .ok_or(OpenTxReaderError::OutOfBound)
    }
    /// Get previous output of input
    /// # Arguments
    /// * `index` absolute index of inputs.
    fn input_previous_output(&self, index: usize) -> Result<OutPoint, OpenTxReaderError> {
    ...
    /// # Arguments
    /// * `index` absolute index of inputs.
    fn input_cell(&self, index: usize) -> Result<CellOutput, OpenTxReaderError> {
    ...
    /// # Arguments
    /// * `index` absolute index of inputs.
    fn group_input_cell(&self, index: usize) -> Result<CellOutput, OpenTxReaderError> {
        if self.group_input_index.len() <= index {
            return Result::Err(OpenTxReaderError::OutOfBound);
        }
    ...
    /// # Arguments
    /// * `index` absolute index of inputs.
    fn input_cell_data(&self, index: usize) -> Result<bytes::Bytes, OpenTxReaderError> {
        let previous_output = self.input_previous_output(index)?;
    ...
    fn output_cell(&self, index: usize) -> Result<CellOutput, OpenTxReaderError> {
        self.transaction
            .output(index)
            .ok_or(OpenTxReaderError::OutOfBound)
    }
    ...
    fn group_output_cell(&self, index: usize) -> Result<CellOutput, OpenTxReaderError> {
    ...
    /// # Arguments
    /// * `index` absolute index of output.
    fn output_cell_data(&self, index: usize) -> Result<bytes::Bytes, OpenTxReaderError> {
        Ok(self
            .transaction
            .outputs_data()
            .get(index)
    ...
    fn cell_dep(&self, index: usize) -> Result<CellDep, OpenTxReaderError> {
        self.transaction
            .cell_deps()
            .get(index)
            .ok_or(OpenTxReaderError::OutOfBound)
    ...
    /// # Arguments
    /// * `index` absolute index of cell deps.
    fn cell_dep_cell(&self, index: usize) -> Result<CellOutput, OpenTxReaderError> {
        let outpoint = self.cell_dep(index)?;
        let cell = self.provider.get_cell(&outpoint.out_point())?;
        Ok(cell)
    }
    fn cell_dep_cell_data(&self, index: usize) -> Result<bytes::Bytes, OpenTxReaderError> {
        let outpoint = self.cell_dep(index)?;
    ...
    pub fn load_cell(
        ...
        index: usize,
        source: OpenTxSource,
    ) -> Result<Vec<u8>, OpenTxReaderError> {
        let cell = match source {
            ...
            _ => Err(OpenTxReaderError::UnsupportSource),
        };
    ...
    pub fn load_cell_data(
        ...
        let data = match source {
            ...
            _ => return Err(OpenTxReaderError::UnsupportSource),
        };
        Ok(data.to_vec())
    }

    pub fn load_input(&self, index: usize) -> Result<Vec<u8>, OpenTxReaderError> {
    ...
    fn load_field_type(
        ...
        match source {
            OpenTxSource::Input => {
                ...
                let d = input.type_();
                if d.is_none() {
                    Result::Err(OpenTxReaderError::ItemMissing)
                } else {
        ...
        match source {
            ...
            OpenTxSource::Outpout => {
                ...
                let d = output
                    .type_()
                    .to_opt()
                    .ok_or(OpenTxReaderError::ItemMissing)?;
    ...
    pub fn load_input_field_out_point(&self, index: usize) -> Result<Vec<u8>, OpenTxReaderError> {
        Ok(self.input(index)?.previous_output().as_slice().to_vec())
    }

    pub fn load_input_field_since(&self, index: usize) -> Result<Vec<u8>, OpenTxReaderError> {
        Ok(self.input(index)?.since().as_slice().to_vec())
    }

    pub fn get_cell(&self, index: usize, is_input: bool) -> Result<CellOutput, OpenTxReaderError> {
